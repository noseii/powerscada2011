...
{
    public class sqlclientUtil
    {
        public static string RunSqlStr(string str,string constr)
        {
            SqlCommand cmd = new SqlCommand(str);
            return RunSql(cmd,constr);
        }
        public static string RunSql(SqlCommand cmd, string constr)
        {
            try
            {
        ...
        public static bool Insert(Object inentity)
        {
                ...
                string sql = @"INSERT INTO {0}
                                ...
                                VALUES 
                                    (
                                         (select isnull(max(Id),0)+1 from {0}),1,getdate(),1,'" + GetMakAdres() + @"',
                                         {2}
                                    )
                              ";
                ...
                string res = RunSql(cmd,Current.constr);
                if (res=="ok")
                {
                    return true;
                }
        ...
        public static bool Delete(Object inentity)
        {
            try
            {
                return RunSql(new SqlCommand("DELETE FROM " + inentity.GetType().Name + " WHERE Id = " + inentity.GetType().GetProperty("Id").GetValue(inentity, null)), Current.constr) != "error";
            }
            catch
            {
                return false;
        ...
        {
            SqlConnection cnn = new SqlConnection(Current.constr);
